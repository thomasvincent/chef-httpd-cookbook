  ...
  module Telemetry
    ...
    def configure_prometheus_exporter(module_path = nil, scrape_uri = '/server-status?auto',
      ...
      if apache_has_prometheus_module?
        ...
        configure_builtin_prometheus_exporter(scrape_uri, telemetry_path, metrics)
      else
        Chef::Log.info('Configuring external Prometheus exporter for Apache')
        configure_external_prometheus_exporter(module_path, scrape_uri, telemetry_path, metrics)
      end
    end
...
<IfModule mod_status.c>
  ...
  ExtendedStatus On
    ...
    # @return [Boolean] True if configured successfully, false otherwise
    def configure_grafana_dashboard(grafana_url, _datasource, api_key = nil)
      require 'uri'
      require 'net/http'
      require 'json'
