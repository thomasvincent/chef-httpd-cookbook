...
module Httpd
  module OpsActions
    ...
    def restore_config(backup_file, force = false)
      ...
      begin
        ...
        if ::File.exist?(metadata_file)
          ...
          if !force && (metadata['platform'] != node['platform'] ||
    ...
    def blue_green_deployment(config_dir = nil, blue_dir = nil, green_dir = nil)
      config_dir ||= case node['platform_family']
                     when 'debian'
                       '/etc/apache2'
                     when 'rhel', 'fedora', 'amazon'
      ...
      FileUtils.cp_r(Dir["#{active_dir}/*"], inactive_dir) if Dir["#{inactive_dir}/*"].empty?
    ...
    def apache_version
      ...
      if cmd.exitstatus.zero?
        match = cmd.stdout.match(%r{version: Apache/(\d+\.\d+\.\d+)}i)
