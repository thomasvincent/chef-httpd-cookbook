...
describe 'Httpd::Telemetry' do
  ...
  describe '#apache_has_prometheus_module?' do
    context 'on Debian systems' do
      ...
      it 'handles errors gracefully' do
        expect(::File).to receive(:exist?).with('/usr/lib/apache2/modules/mod_prometheus_exporter.so').and_raise(
  ...
  describe '#configure_grafana_dashboard' do
    ...
    context 'with API key provided' do
      ...
      before do
        allow(URI).to receive(:parse).and_return(double('uri', host: 'grafana', port: 3000, scheme: 'http',
                                                               request_uri: '/api/dashboards/db'))
