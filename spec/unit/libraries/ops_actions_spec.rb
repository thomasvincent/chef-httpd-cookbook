...
describe Httpd::OpsActions do
  ...
  before do
    ...
    allow(subject).to receive(:shell_out).and_return(double('shellout', exitstatus: 0,
                                                                        stdout: 'Apache version: Apache/2.4.41', stderr: ''))
  ...
  describe '#backup_config' do
    ...
    it 'creates a tar.gz archive of the configuration directory' do
      expect(subject).to receive(:shell_out).with('tar -czf /var/backups/httpd/apache-config-20250516-120000.tar.gz -C /etc /apache2').and_return(double(
                                                                                                                                                    'shellout', exitstatus: 0, stderr: ''
                                                                                                                                                  ))
    ...
    it 'adds a label to the backup filename if provided' do
      expect(subject).to receive(:shell_out).with('tar -czf /var/backups/httpd/apache-config-20250516-120000-mylabel.tar.gz -C /etc /apache2').and_return(double(
                                                                                                                                                            'shellout', exitstatus: 0, stderr: ''
                                                                                                                                                          ))
    ...
    it 'returns nil if creating the archive fails' do
      allow(subject).to receive(:shell_out).and_return(double('shellout', exitstatus: 1,
  ...
  describe '#restore_config' do
    ...
    it 'extracts the backup archive to a temporary directory' do
      expect(subject).to receive(:shell_out).with('tar -xzf /var/backups/httpd/backup.tar.gz -C /tmp/apache-restore').and_return(double(
                                                                                                                                   'shellout', exitstatus: 0, stderr: ''
                                                                                                                                 ))
    ...
    it 'validates the restored configuration' do
      expect(subject).to receive(:shell_out).with('apache2ctl -t').and_return(double('shellout', exitstatus: 0,
                                                                                                 stderr: ''))
    ...
    it 'rolls back to previous configuration if validation fails' do
      allow(subject).to receive(:shell_out).with('apache2ctl -t').and_return(double('shellout', exitstatus: 1,
                                                                                                stderr: 'Syntax error'))
  ...
  describe '#blue_green_deployment' do
    ...
    it 'validates the inactive environment configuration' do
      allow(::File).to receive(:symlink?).with('/etc/apache2').and_return(true)
      allow(::File).to receive(:readlink).with('/etc/apache2').and_return('/etc/apache2-blue')
      expect(subject).to receive(:shell_out).with('APACHE_CONFDIR=/etc/apache2-green apache2ctl -t').and_return(double(
                                                                                                                  'shellout', exitstatus: 0, stderr: ''
                                                                                                                ))
    ...
    it 'returns active environment if validation fails' do
      allow(::File).to receive(:symlink?).with('/etc/apache2').and_return(true)
      allow(::File).to receive(:readlink).with('/etc/apache2').and_return('/etc/apache2-blue')
      allow(subject).to receive(:shell_out).with('APACHE_CONFDIR=/etc/apache2-green apache2ctl -t').and_return(double(
                                                                                                                 'shellout', exitstatus: 1, stderr: 'Syntax error'
                                                                                                               ))
      expect(Chef::Log).to receive(:error).with(/syntax errors/)
    ...
    it 'verifies Apache started successfully' do
      allow(::File).to receive(:symlink?).with('/etc/apache2').and_return(true)
      allow(::File).to receive(:readlink).with('/etc/apache2').and_return('/etc/apache2-blue')
      expect(subject).to receive(:shell_out).with('systemctl is-active apache2 || service apache2 status').and_return(double(
                                                                                                                        'shellout', exitstatus: 0
                                                                                                                      ))
    ...
    it 'rolls back if Apache fails to start with new configuration' do
      allow(::File).to receive(:symlink?).with('/etc/apache2').and_return(true)
      allow(::File).to receive(:readlink).with('/etc/apache2').and_return('/etc/apache2-blue')
      allow(subject).to receive(:shell_out).with('systemctl is-active apache2 || service apache2 status').and_return(double(
                                                                                                                       'shellout', exitstatus: 1
                                                                                                                     ))
      allow(subject).to receive(:sleep)
  ...
  describe '#apache_version' do
    it 'parses Apache version from command output' do
      allow(subject).to receive(:shell_out).with('apache2 -v').and_return(double('shellout', exitstatus: 0,
                                                                                             stdout: 'Server version: Apache/2.4.41 (Ubuntu)'))
